...
class Cameras(object):
    def __init__(
            ...
            intrinsic_matrices=None, viewing_angles_x=None, viewing_angles_y=None, image_h=256, image_w=256):
        ...
        if intrinsic_matrices is None:
            ...
            fx = 1 / torch.tan(viewing_angles_x / 2)
            fy = 1 / torch.tan(viewing_angles_y / 2)
            tx = image_w / 2
            ty = image_h / 2
    ...
    def __call__(self, vertices):
        ...
        if self.rotation_matrices.ndim == 3:
            vertices = torch.matmul(vertices, self.rotation_matrices, transb=True)
        elif self.rotation_matrices.ndim == 2:
            vertices = torch.matmul(vertices, self.rotation_matrices, transb=True)

        vertices = torch.matmul(vertices, self.intrinsic_matrices, transb=True)
